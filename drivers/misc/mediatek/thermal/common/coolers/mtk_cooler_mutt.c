...
#define MUTT_ACTIVED_OFFSET		8
#define MUTT_SUSPEND_OFFSET		16

#define MUTT_ACTIVED_FILTER		0x0000FF00
...
#define MUTT_ACTIVED_OFFSET		16
#define MUTT_SUSPEND_OFFSET		24

#define MUTT_ACTIVED_FILTER		0x00FF0000
				...
				unsigned long *state)
{
	*state = cl_dev_noIMS_state;
	mtk_cooler_mutt_dprintk(
			"mtk_cl_noIMS_get_max_state() %s %lu (0: md IMS OK;  1: md no IMS)\n",
			cdev->type, *state);
				...
				unsigned long state)
{
	...
	if (cl_dev_mdoff_state == 1) {
		mtk_cooler_mutt_dprintk(
			"mtk_cl_noIMS_set_cur_state():  MD STILL OFF!!\n");
	...
	if ((state >= 0) && (state <= 1))
		...
		mtk_cooler_mutt_dprintk(
			"mtk_cl_noIMS_set_cur_state(): Invalid input(0: md IMS OK; 1: md no IMS)\n");
		return 0;
	}
...
static void mtk_cl_mutt_set_mutt_limit(void)
{
	...
	for (; i < MAX_NUM_INSTANCE_MTK_COOLER_MUTT; i++) {
		...
		if (curr_state == 1) {
			...
			int limit = 0;
			active =
				(cl_mutt_param[i] & MUTT_ACTIVED_FILTER) >>
					MUTT_ACTIVED_OFFSET;
			suspend =
...
static int _mtk_cl_mutt_proc_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "curr_limit %x, noIMS: %d, mdoff: %d\n",
		...
		active = (cl_mutt_param[i] & MUTT_ACTIVED_FILTER) >>
				MUTT_ACTIVED_OFFSET;
...
struct file *filp, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (scan_count >= 1) {
		...
		&& mutt0_s >= 100 && mutt0_s <= 25500)
			...
			((mutt0_a / 100) << MUTT_ACTIVED_OFFSET) |
		...
		&& mutt1_s >= 100 && mutt1_s <= 25500)
			...
			((mutt1_a / 100) << MUTT_ACTIVED_OFFSET) |
			MUTT_ENABLE_IMS_ENABLE;
		...
		&& mutt2_s >= 100 && mutt2_s <= 25500)
			...
			((mutt2_a / 100) << MUTT_ACTIVED_OFFSET) |
			MUTT_ENABLE_IMS_ENABLE;
		...
		&& mutt3_s >= 100 && mutt3_s <= 25500)
			...
			((mutt3_a / 100) << MUTT_ACTIVED_OFFSET) |
			MUTT_ENABLE_IMS_ENABLE;
...
struct file *filp, const char __user *buffer, size_t count, loff_t *data)
{
	...
	&& mutt_s >= 100 && mutt_s <= 25500)
		...
		((mutt_a / 100) << MUTT_ACTIVED_OFFSET);
